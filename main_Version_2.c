...
int main(void)
{
	...
	DDRA = 0x00;										//Inputs
	DDRD = 0xFF;										//Outputs
	DDRC = 0x10;										//00010000 (0x10) - TestDataInput(out from ATmega) and TestDataOutput/TestModeSelect/TestClock(in to ATmega).
	ADCSRA = 0x80;										//ADEN = 1		//ADC Enable ADEN.
	ADMUX = 0x20;										//ADLAR = 1		//To only use ADCH.
	
	double Current_Data[3];								//Variables have underscores in their name, functions does not!
	double Out_Data[3];
...
//------------------------------------------------------------------------------------------------------------------	
	void CheckReflexSensors(int in)
	{
...
//------------------------------------------------------------------------------------------------------------------
    while (1) //Self running mode active = 1 
    {
		...
		//Checks the difference between the most Left sensor and the most Right sensor -----------------------------------------------
