...
//Out || Out-temp || Time
int Current_Data[7];
int Out_Data[7]; // 0 = tejpsensor, 1 = wheel, 2 = wheel, 3 = kollision, 4 = sv�ng variabel, 5 = vinkel diff, 6 = autonom knapp , 7 = pins between two samples(mikro*(m/s) need to divide with 1e6)
...
int main(void)
{
	...
	while (1) //always run because we need to loop back to while(pind5 ==1), need to be able to turn the switch on and off multiple times. 
	{
		...
		while (PIND5 == 1)//Self running mode active = 1  (If switch is active :: pin PD5 :: If PD5 = 5V then Auto else if PD5 = GND then manual)
		{
