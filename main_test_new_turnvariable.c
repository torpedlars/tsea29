...
volatile int Out_Data[9]; // 0 = tejpsensor, 1 = wheel, 2 = wheel, 3 = kollision, 4 = sv�ng variabel, 5 = V�rde p� h�gsta tejp sensor, 6 = autonom knapp , 7 = pins between two samples(mikro*(m/s) need to divide with 1e6), 8 = n�st h�gsta tejp sensor
...
//------------------------------------------------------------------------------------------------------------------
uint8_t IdentifyTurn(int Above_Tape[]) // har korrigerat detta enda i den här koden annars är det den nyaste som finns på git och på mikrokontrollern
{
	volatile uint8_t Turn_left = 0;
	volatile uint8_t Turn_right = 0;
	volatile uint8_t tmp = 0;
	if ( Above_Tape[2] == 1 && Above_Tape[3] == 1)
	{
		if (Above_Tape[5] == 1)
		{
			Turn_left = 0x80;
	...
	if ( Above_Tape[7] == 1 && Above_Tape[8] == 1)
	{
		if (Above_Tape[5] == 1)
		{
			Turn_right = 0x40;
...
int main(void)
{
	...
	while (1) //always run because we need to loop back to while(pind5 ==1), need to be able to turn the switch on and off multiple times. 
	{
		...
		while (Current_Data[6] == 1)//Self running mode active = 1  (If switch is active :: pin PD5 :: If PD5 = 5V then Auto else if PD5 = GND then manual)
		{
			...
			for (int i = 0; i < 11; i++) // kan bli problem n�r alla sensorer �r t�ckta. kanske m�ste l�sa detta.
			{
