...
uint8_t Average_wheel_pins = 0;
//IR sensor
double IR_Sensor;

//Out || Out-temp || Time
volatile int Current_Data[8];
int Out_Data[8]; // 0 = tejpsensor, 1 = wheel, 2 = wheel, 3 = kollision, 4 = sv�ng variabel, 5 = vinkel diff, 6 = autonom knapp , 7 = pins between two samples(mikro*(m/s) need to divide with 1e6)
...
ISR(TWI_vect)
{
	...
	if (Outdata_counter > 8)
	{
		Outdata_counter=0;
		Average_wheel_pins = 0;
	}
	switch ( status )
	{
...
//------------------------------------------------------------------------------
			...
			Average_wheel_pins = (Wheel1_pins_tmp + Wheel2_pins_tmp)/2;
			Current_Data[7] = Average_wheel_pins;
			//----------------------------------------------------------------------------------------------------------
