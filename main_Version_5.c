...
ISR(TWI_vect)											//Vector 27 (TWI)
{
	...
	{
		TWDR = Out_Data[i];
		while(TWINT == 0)								//Wait for TWINT to get set to 1 (high) // osäker om det här är rätt, borde vi inte titta på TWSR(status register) och vänta
														//på att vi blir färdiga med en byte?
		{}
